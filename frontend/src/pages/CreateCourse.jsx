...
import { PlusIcon, BookOpenIcon, PencilIcon, TrashIcon } from '../components/icons';
                ...
                <form onSubmit={handleInitialSubmit} className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md">
                    ...
                    <div className="mt-8">
                        <button type="submit" disabled={loading} className="w-full flex justify-center py-3 px-4 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 disabled:opacity-50">
                            {loading ? "A Criar..." : "Próximo: Adicionar Módulos"}
                        </button>
                    </div>
                </form>
            </div>
...
function CourseBuilder() {
    ...
    const handleAddModule = async (e) => {
        ...
        try {
            await api.post(`/api/courses/${courseId}/modules`, { title: newModuleName, order: modules.length + 1 });
            setNewModuleName('');
            fetchCourseStructure();
        } catch (error) {
    ...
    const startEditingModule = (module) => {
        ...
        setEditingModuleText(module.title);
    };
    
    const handleUpdateModule = async (e) => {
        e.preventDefault();
        if (!editingModule || !editingModuleText.trim()) return;
        try {
            await api.put(`/api/modules/${editingModule.id}`, { title: editingModuleText });
    ...
    const handleDeleteModule = async (moduleId) => {
        if (window.confirm('Tem a certeza de que quer excluir este módulo e todas as suas aulas?')) {
            try {
    ...
    if (loading) return <Layout pageTitle="A Carregar..."><div>A carregar construtor de curso...</div></Layout>;
        ...
        <Layout pageTitle={`Construtor: ${course?.title}`}>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                <div className="text-center p-4 bg-green-100 text-green-800 rounded-lg">
                    <p>Curso criado com sucesso! Agora, adicione e organize os seus módulos e aulas.</p>
                </div>
                {modules.map(module => (
                    <div key={module.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md">
                        <div className="flex justify-between items-center">
                            {editingModule?.id === module.id ? (
                                <form onSubmit={handleUpdateModule} className="flex-grow flex gap-2">
                                    <input
                                        type="text"
                                        ...
                                        autoFocus
                                    />
                                    <button type="submit" className="px-4 py-2 bg-emerald-500 text-white rounded-md">Salvar</button>
                                    <button type="button" onClick={() => setEditingModule(null)} className="px-4 py-2 bg-gray-200 rounded-md">Cancelar</button>
                                </form>
                            ) : (
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                                <button onClick={() => startEditingModule(module)} className="p-2 text-gray-400 hover:text-blue-600">
                                    <PencilIcon className="w-5 h-5" />
                                </button>
                                <button onClick={() => handleDeleteModule(module.id)} className="p-2 text-gray-400 hover:text-red-600">
...
function CreateOrEditCourse() {
    ...
    // Se há um ID na URL, estamos a editar (mostra o construtor).
    // Se não, estamos a criar (mostra a etapa inicial).
